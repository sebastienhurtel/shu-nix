...
in
{
  ...
  config = lib.mkIf cfg.enable {
    home-manager.users.${username} = {
      home.file."${hmConfig.xdg.configHome}/zsh/.p10k.zsh".source = ./p10k.zsh;

      age.secrets.shScripts = {
        file = "${self}/secrets/shScripts.age";
        path = "${hmConfig.xdg.configHome}/zsh/vpn.sh";
      };
      ...
      programs = {
        ...
        zsh = {
          ...
          initContent = ''
            ...
            test -f ${hmConfig.xdg.configHome}/zsh/.p10k.zsh && source ${hmConfig.xdg.configHome}/zsh/.p10k.zsh
            # Custom scripts
            test -f ${hmConfig.xdg.configHome}/zsh/vpn.sh && source ${hmConfig.xdg.configHome}/zsh/vpn.sh
            pssh () { ${pkgs.sshpass}/bin/sshpass -f <(${pkgs.pass}/bin/pass show free/ssh) ssh "$@"; }
            compdef pssh='ssh'
          '';
          envExtra = ''
            # Add doom to path
            export PATH="${hmConfig.xdg.configHome}/emacs/bin:$PATH"
          '';
          oh-my-zsh = {
            enable = true;
            custom = "${hmConfig.xdg.configHome}/zsh";
            plugins = [
              "ansible"
              "clab"
              "docker"
